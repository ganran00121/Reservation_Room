...
export default {
  ...
  setup() {
    ...
    const openEdit_modal = (info) => {
      ...
      newEvent_edit.value.start_date = convertDate(newEvent_edit.value.start_date)
...
<template>
  <admin_reser v-if="showModal" :form="newEvent" @closeModal="closeModal_add" @saveAppt="saveAppt_add" />
  <EditCourse v-if="show_edit" :form="newEvent_edit" @closeModal="closeModal_edit" @saveAppt="saveAppt_edit" />
      ...
      <table class="table-auto w-full rounded-xl ">
        ...
        <tbody v-for="(items, index) in data" :key="index" class="text-center divide-y ">
          <tr v-if="index + 1 <= this.count && index + 1 >= this.number" class="hover:bg-gray-50 border-b-2 ml-3"
            ...
            <td>{{ items.course_name }}<br>{{ items.course_id }} ( {{ items.course_section }} ) {{ items.course_type }}
            </td>
            <td>{{ items.time }}<br>{{ items.day_of_week }}</td>
            <td>{{ items.start_date }} - {{ items.end_date }}</td>
            <td>
              {{ items.course_instructor }}<br>{{ items.email }}
            </td>
            <td class="mr-8 ">
              <div class="flex justify-end  mr-8">
                <button class="flex rounded-xl bg-emerald-400 p-1 px-3 text-white" @click="openEdit_modal(items)">
                ...
                <button class="flex rounded-xl bg-red-400 p-1 px-2 text-white ml-3" @click="Delete_course(items)">
